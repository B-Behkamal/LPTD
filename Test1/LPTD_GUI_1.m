function varargout = LPTD_GUI_1(varargin)
% LPTD_GUI_1 MATLAB code for LPTD_GUI_1.fig
%      LPTD_GUI_1, by itself, creates a new LPTD_GUI_1 or raises the existing
%      singleton*.
%
%      H = LPTD_GUI_1 returns the handle to a new LPTD_GUI_1 or the handle to
%      the existing singleton*.
%
%      LPTD_GUI_1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in LPTD_GUI_1.M with the given input arguments.
%
%      LPTD_GUI_1('Property','Value',...) creates a new LPTD_GUI_1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before LPTD_GUI_1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to LPTD_GUI_1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help LPTD_GUI_1

% Last Modified by GUIDE v2.5 30-Jan-2022 11:21:52

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @LPTD_GUI_1_OpeningFcn, ...
    'gui_OutputFcn',  @LPTD_GUI_1_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before LPTD_GUI_1 is made visible.
function LPTD_GUI_1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to LPTD_GUI_1 (see VARARGIN)

% Choose default command line output for LPTD_GUI_1
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes LPTD_GUI_1 wait for user response (see UIRESUME)
...
function varargout = LPTD_GUI_1_OutputFcn(hObject, eventdata, handles)
...
et_topology_strand = findobj(0, 'tag', 'et_topology_strand');
topology=get(et_topology_strand,'String');
topology=string(topology);
protein_data.topology_strands=topology;
radiobutton_helix = findobj(0, 'tag', 'radiobutton_helix');
...
if sheet_info==1
    final_topology=LPTD_Function_1(protein_data,helix_info,sheet_info)
...
if get(check_helix, 'Value') == 1
    ...
    chain=string(chain);
    protein_data.chain=chain;
    et_topology_helix = findobj(0, 'tag', 'et_topology_helix');
    topology_helix=get(et_topology_helix,'String');
    topology_helix=string(topology_helix);
    protein_data.topology_hlces=topology_helix;
    radiobutton_helix = findobj(0, 'tag', 'radiobutton_helix');
    helix_info=get(radiobutton_helix,'value');
    helix_info=double(helix_info(1));
    protein_data.helix_info=helix_info;
...
[final_topology, results]=LPTD_Function_1(protein_data,helix_info,sheet_info);
...
precision_box = findobj(0, 'tag', 'precision');
set(precision_box, 'String', num2str(results.precision))
recall_box = findobj(0, 'tag', 'recall');
set(recall_box, 'String', num2str(results.recall))
fmeasure_box = findobj(0, 'tag', 'fmeasure');
set(fmeasure_box, 'String', num2str(results.f_measure))
accuracy_box = findobj(0, 'tag', 'accuracy');
set(accuracy_box, 'String', num2str(results.accuracy))
rank_box = findobj(0, 'tag', 'rank');
set(rank_box, 'String', num2str(results.rank))
